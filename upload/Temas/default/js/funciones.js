...
function remind_password(){
    $('#login_box').hide();
    mydialog.alert('Aviso','Lo sentimos esta función aun no está disponible, contacta al administrador para cambiar tu password.');
}
/** MODERACION DE POST, COMENTARIOS *
var moderar = {
    posts: {
        borrar:function(aceptar){
    	if(!aceptar){
        	$.ajax({
        		type: 'POST',
        		...
        		success: function(h){
        			...
        			mydialog.body(h);
        			mydialog.buttons(true, true, 'Borrar', 'moderar.posts.borrar(1);', true, false, true, 'Cancelar', 'close', true, true);
        	...
        	$.ajax({
        		type: 'POST',
        		url: global_data.url + '/moderacion-posts.php?do=borrar',
        		data: gget('postid', true) + '&razon=' + razon + '&razon_desc=' + razon_desc,
        		success: function(h){
        			switch(h.charAt(0)){
        				case '0': //Error
        					...
        					break;
        				case '1':
        					mydialog.alert('Post Borrado', '<div class="dialog_box">' + h.substring(3) + '</div>', true);
        					break;
        			}
        		},
        		complete: function(){
