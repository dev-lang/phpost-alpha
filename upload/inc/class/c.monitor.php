...
class tsMonitor extends tsDatabase {
	...
	*/
	public function __construct(){
         ...
         * AVISOS
        */ 
		$query = $this->query("SELECT COUNT(av_id) AS total FROM u_avisos WHERE user_id = {$tsUser->uid} AND av_read = 0");
		$data = $this->fetch_assoc($query);
		$this->free($query);
    ...
    */
    function setAviso($user_id, $subject = '(sin asunto)', $body, $type = 0){
        ...
        $query = $this->select("u_miembros","user_baneado","user_id = {$user_id}","",1);
        $data = $this->fetch_assoc($query);
        $this->free($query);
        ...
        # INSERTAMOS EL AVISO
        $date = time();
        if($this->insert("u_avisos","user_id, av_subject, av_body, av_date, av_type","{$user_id}, '{$subject}', '{$body}', {$date}, {$type}")) return true;
        else return false;
    }
    /**
     * @name getAvisos
    ...
    public function getAvisos(){
        ...
        global $tsUser;
        # OBTENERMOS
		$query = $this->query("SELECT * FROM u_avisos WHERE user_id = {$tsUser->uid}");
    ...
    public function readAviso($av_id){
        ...
        global $tsUser;
        # OBTENEMOS
		$query = $this->query("SELECT * FROM u_avisos WHERE av_id = {$av_id}");
		$data = $this->fetch_assoc($query);
		$this->free($query);
        # RETURN
        if(empty($data['av_id']) || $data['user_id'] != $tsUser->uid) return 'El aviso no existe';
        else {
            $this->update("u_avisos","av_read = 1","av_id = {$av_id}");
    ...
    public function delAviso($av_id){
        ...
        global $tsUser;
        # OBTENEMOS
		$query = $this->query("SELECT user_id FROM u_avisos WHERE av_id = {$av_id}");
		$data = $this->fetch_assoc($query);
		$this->free($query);
        # RETURN
        if(empty($data['user_id']) || $data['user_id'] != $tsUser->uid) return false;
        else {
            $this->delete("u_avisos","av_id = {$av_id}");
            return true;
        }
    }
	/**
	...
	*/
	...
	public function getNotificaciones($unread = false){
		...
		// SI VA AL MONITOR ENTONCES ACTUALIZAMOS PARA QUE YA NO SE VEAN EN EL MENUBAR
			...
			$cuery = $this->query("SELECT COUNT(follow_id) AS total FROM u_follows WHERE f_user = {$tsUser->uid} AND f_type = 2");
			$stats = $this->fetch_assoc($cuery);
			$this->free($cuery);
			// ARMAMOS PARA MAS COMODIDAD
            $dataDos['stats']['posts'] = empty($stats['total']) ? 0 : $stats['total'];
            # CARGO LOS FILTROS
            $cuery = $this->select("u_portal","c_monitor","user_id = {$tsUser->uid}","",1);
            $filtros = $this->fetch_assoc($cuery);
            $this->free($cuery);
            //
		...
		$this->free($query);
        // ACTUALIZAMOS 
        if($this->show_type == 1) $this->update("u_monitor","not_menubar = {$not_del}","user_id = {$tsUser->uid} AND not_menubar > 0");
        else $this->update("u_monitor","not_menubar = 0, not_monitor = 0","user_id = {$tsUser->uid} AND not_monitor = 1");
	...
	*/
	function makeConsulta($data){
		...
		switch($data['not_type']){
            ...
            case 14:
                if($data['obj_dos'] == 2)
                    return "SELECT pub_id AS obj_uno FROM u_muro_comentarios WHERE cid = {$data['obj_uno']}";
                else return array('valie' => 'hack');
            break;
		}
	}
    /**
	...
	*/
    ...
    public function setFiltro(){
        ...
        $query = $this->select("u_portal","c_monitor","user_id = {$tsUser->uid}","",1);
        $data = $this->fetch_assoc($query);
        $this->free($query);
        # PROSESAMOS
        ...
        $this->update("u_portal","c_monitor = '{$filtros}'","user_id = {$tsUser->uid}");
        //
        return true;
    }
    /**
    ...
    private function allowNotifi($type, $user_id){
        # CONSULTAMOS
        $query = $this->select("u_portal","c_monitor","user_id = {$user_id}","",1);
        $data = $this->fetch_assoc($query);
        $this->free($query);
        # PROSESAMOS
        $filtro_id = 'f'.$type;
