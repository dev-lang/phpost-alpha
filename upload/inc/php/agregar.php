...
\*********************************/
	...
	if(is_numeric($action)){
		...
		if(!empty($_POST['titulo'])){
		  $post_save = $tsPosts->savePost();
			if($post_save == 1) {
				...
				$query = $tsdb->query("SELECT c.c_seo FROM p_categorias AS c WHERE c.cid = $tsCat LIMIT 1");
				$tsCat = $tsdb->fetch_assoc($query);
				$tsdb->free($query);
				//
		...
		if($tsPost > 0) {
			...
			$tsdb->free($query);
			//
			$smarty->assign("tsAviso",array('titulo' => 'Bien!', 'mensaje' => "El post <b>{$_POST['titulo']}</b> fue agregado.", 'but' => 'Acceder al post', 'link' => "{$tsCore->settings['url']}/posts/{$tsCat['c_seo']}/$tsPost/{$tsCore->setSEO($_POST['titulo'])}.html"));
		} elseif($tsPost == -1){
